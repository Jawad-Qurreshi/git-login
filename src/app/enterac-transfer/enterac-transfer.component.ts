...
@Component({
  selector: 'app-enterac-transfer',
  templateUrl: './enterac-transfer.component.html',
  styleUrls: ['./enterac-transfer.component.css']
})
export class EnteracTransferComponent implements OnInit {
  ...
  @ViewChild('enteracpaym') inputenteracpay: ElementRef;

  form: FormGroup;
  enteractextention: any;
  lastenterac: any;
  enteracrand: any;
  errenterac: boolean;
  upform: any;
  ...
  upload_enterac(event) {
    ...
    if (event.target.files && event.target.files.length > 0) {
      ...
      this.enteractextention = file.name.split(".");
      this.lastenterac = this.enteractextention[this.enteractextention.length - 1];
      this.enteracrand = this.getrand(0, 1000000);

      if (this.lastenterac != "png" && this.lastenterac != "jpg" && this.lastenterac != "jpeg") {
        alertify.error("Please upload only png, jpg and jpeg file");
        this.errenterac = false;
      }
    }
  }

  enteracpay = function (data) {

    const formModel = this.form.value;

    if (formModel.payment_img == null) {
      this.errenterac = false;
      alertify.error("Please Choose Enterac Transfer Photo");
    } else {
      if (this.lastenterac != "png" && this.lastenterac != "jpg" && this.lastenterac != "jpeg") {
        alertify.error("Please upload only png, jpg and jpeg file of Enterac Transfer photo");
        this.errenterac = false;
      } else {
        this.errenterac = true;
      }
    }

    if (this.errenterac == true) {
      ...
      var finaldata = {
        ...
        "payment_img_photo": this.lastenterac + formModel.payment_img.filename,
