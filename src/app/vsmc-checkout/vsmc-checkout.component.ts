...
@Component({
  selector: 'app-vsmc-checkout',
  templateUrl: './vsmc-checkout.component.html',
  styleUrls: ['./vsmc-checkout.component.css']
})
export class VsmcCheckoutComponent implements OnInit {
  ...
  getToken() {
    ...
    let valid_name = /^((4\d{3})|(5[1-5]\d{2})|(6011))-?\d{4}-?\d{4}-?\d{4}|3[4,7]\d{13}$/;
    let chkcardno = valid_name.test(this.cardNumber);
    ...
    if (this.cardNumber == "" || chkcardno == false) {
